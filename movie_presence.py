...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        tmdb_id = None
        if movie_query.lower().startswith("id:"):
            try:
                tmdb_id = int(movie_query[3:].strip())
            except ValueError:
                print("✗ Invalid TMDB ID")
                sys.exit(1)
        elif movie_query.isdigit():
            tmdb_id = int(movie_query)
        if tmdb_id:
        ...
        year = None
        title = movie_query
        if movie_query.endswith(")") and "(" in movie_query:
            try:
                year_start = movie_query.rfind("(")
                year_str = movie_query[year_start + 1:-1].strip()
                if year_str.isdigit() and len(year_str) == 4:
                    year = int(year_str)
                    title = movie_query[:year_start].strip()
            except:
                pass
